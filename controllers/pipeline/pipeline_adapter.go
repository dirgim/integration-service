...
func (a *Adapter) EnsureApplicationSnapshotPassedAllTests() (results.OperationResult, error) {
	...
	if pipelineType != "test" {
		...
		integrationPipelineRuns, err := a.getAllPipelineRunsForApplicationSnapshot(existingApplicationSnapshot)
		if err != nil {
		...
		allIntegrationPipelineRunsFinished := true
		for _, integrationPipelineRun := range *integrationPipelineRuns {
			...
			if err != nil {
				...
				allIntegrationPipelineRunsFinished = false
			}
			if !integrationPipelineRun.Status.GetCondition(apis.ConditionSucceeded).IsTrue() {
				a.logger.Info("Integration PipelineRun has not finished yet",
					"PipelineRun.Name", integrationPipelineRun.Name, "PipelineRun.Namespace", integrationPipelineRun.Namespace)
				allIntegrationPipelineRunsFinished = false
			} else if !pipelineRunOutcome {
		...
		if allIntegrationPipelineRunsPassed {
			a.logger.Info("All Integration PipelineRuns succeeded, marking ApplicationSnapshot as succeeded",
				"Application.Name", a.application.Name,
				"ApplicationSnapshot.Name", existingApplicationSnapshot.Name)
			existingApplicationSnapshot.MarkSucceeded()
			err := a.updateSnapshotStatus(existingApplicationSnapshot)
			if err != nil {
				return results.OperationResult{}, err
			}
		} else if allIntegrationPipelineRunsFinished {
			a.logger.Info("Some tests within Integration PipelineRuns failed, marking ApplicationSnapshot as failed",
				"Application.Name", a.application.Name,
				"ApplicationSnapshot.Name", existingApplicationSnapshot.Name)
			existingApplicationSnapshot.MarkFailed(appstudioshared.ApplicationSnapshotReasonTestsFailed, "Some integration tests failed")
			err := a.updateSnapshotStatus(existingApplicationSnapshot)
			if err != nil {
				return results.OperationResult{}, err
			}
...
// getAllPipelineRunsForApplicationSnapshot loads from the cluster all Integration PipelineRuns associated with the given ApplicationSnapshot.
// If the Application doesn't have any IntegrationTestScenarios or this is not found in the cluster, an error will be returned.
func (a *Adapter) getAllPipelineRunsForApplicationSnapshot(applicationSnapshot *appstudioshared.ApplicationSnapshot) (*[]tektonv1beta1.PipelineRun, error) {
	...
	for _, integrationTestScenario := range *integrationTestScenarios {
		if a.pipelineRun.Labels["test.appstudio.openshift.io/scenario"] != integrationTestScenario.Name {
			integrationPipelineRun, err := a.getPipelineRunForApplicationSnapshotAndScenario(applicationSnapshot, &integrationTestScenario)
			if err != nil {
				return nil, err
			}
...
// updateSnapshotStatus updates the status of the PipelineRun being processed.
func (a *Adapter) updateSnapshotStatus(snapshot *appstudioshared.ApplicationSnapshot) error {
	return a.client.Status().Update(a.context, snapshot)
}
